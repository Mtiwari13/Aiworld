...
if (!class_exists('ai_blog_About')) {
	...
	class ai_blog_About
	{
		 ...
		 *
		 * @var ai_blog_About $instance The  TI_About_Page instance.
		 */
		...
		{
			if (!isset(self::$instance) && !(self::$instance instanceof ai_blog_About)) {
				self::$instance = new ai_blog_About;
		...
		public function ti_about_page_render()
		{
			...
			if (!empty($welcome_title) || !empty($welcome_content) || !empty($this->tabs)) {
				...
				echo '<a href="' . esc_url('https://blog.wpteamx.com/ai/ai-demos/') . '" target="_blank" class="wp-badge epsilon-welcome-logo"></a>';
		...
		public function free_pro()
		{
			...
			if (!empty($free_pro)) {
				if (!empty($free_pro['free_theme_name']) && !empty($free_pro['pro_theme_name']) && !empty($free_pro['features']) && is_array($free_pro['features'])) {
					...
					echo '<tr><td style="border-top:0;"></td><td style="border-top:0;"></td><td style="border-top:0;" class="only-pro"><a href="' . esc_url('https://blog.wpteamx.com/ai/ai-demos/') . '" target="_blank" >';
					echo esc_html('Ai Blog Pro Live Preview', 'ai-blog');
					echo '</a></td></tr>';
			...
			if ('appearance_page_' . $this->theme_slug . '-welcome' == $hook_suffix) {
				...
				wp_localize_script(
					...
					array(
						...
						'activating_string'        => __('Activating', 'ai-blog')
					)
				);
			}
		}
